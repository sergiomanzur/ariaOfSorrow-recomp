...
- **HD Character Sprites & Spritesheet Pipeline**: High-definition cel-shaded character sprites for Soma Cruz (Idle, Walk, Jump, Attack, Crouch) with real-time pose tracking, subpixel positioning, facing orientation, and an instant on/off toggle via in-game settings (`Graphics -> HD Character Sprites`), `H` hotkey, or config (`hd_sprites = true/false`), strictly preserving faithful native GBA sprites by default (Prime Directive 1).
